    ...
    def forward(self, G, feat_dict):
        ...
        for srctype, etype, dsttype in G.canonical_etypes:
            ...
            if self.weight[local_etype].in_features == feat_dict[srctype].shape[1]:
                ...
                Wh = torch.zeros([1, self.weight[local_etype].out_features]).to(
                    G.device
                )
...
class HetrgcnNet(nn.Module):
    def __init__(
        ...
        super(HetrgcnNet, self).__init__()
        n_node_types = 1
        etypes = 5
        canonical_etypes = [
            (str(s), str(r), str(t))
            for r in range(etypes)
            for s in range(n_node_types)
            for t in range(n_node_types)
        ]
        node_dict = {str(i): i for i in range(n_node_types)}
        edge_dict = {et: str(i) for i, et in enumerate(canonical_etypes)}
        ...
        for _ in range(n_layers):
            self.layers.append(HeteroRGCNLayer(hidden_dim, hidden_dim, edge_dict))
        self.out = nn.Linear(hidden_dim, out_dim)

        self.pools = nn.ModuleList()
        self.linears_prediction = nn.ModuleDict(
            {k: nn.ModuleList() for k, _ in node_dict.items()}
        )

        # Define pooling readout layers
        for layer in range(n_layers + 1):
            for k, molst in self.linears_prediction.items():
                self.linears_prediction[k].append(nn.Linear(hidden_dim, out_dim))
    ...
    def forward(self, G, h=None):
        ...
        for i in range(self.n_layers):
            ...
            for k, v in h.items():
                if h[k].shape[0] > 0:
                    out_h[k] = self.linears_prediction[k][i](
                        self.pools[i](G, h, ntype=k)
                    )
                else:
                    out_h[k] = h[k]
            h_list.append(out_h)
