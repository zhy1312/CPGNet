...
class StainNormalizer(object):
    ...
    def transform(self, images):
        ...
        transform_mat = np.matmul(
            opt_cd_mat * opt_w_mat,
            np.linalg.inv(self._template_dc_mat * self._template_w_mat),
        )

        od = -np.log((np.asarray(images, np.float16) + 1) / 256.0)
        normed_od = np.matmul(od, transform_mat)
        normed_images = np.exp(-normed_od) * 256 - 1

        return np.around(np.maximum(np.minimum(normed_images, 255), 0)).astype(np.uint8)
    ...
    def sampling_data(self, images):
        ...
        pixels = pixels[
            np.random.choice(pixels.shape[0], min(self._pn * 20, pixels.shape[0]))
        ]
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for ep in range(self._epoch):
                for step in range(self._step_per_epoch):
                    sess.run(
                        target,
                        {input_od: od_data[step * self._bs : (step + 1) * self._bs]},
                    )
            opt_cd = sess.run(cd)
            opt_w = sess.run(w)
        return opt_cd, opt_w
