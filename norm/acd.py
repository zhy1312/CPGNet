...
init_varphi = np.asarray([[0.294, 0.110, 0.894], [0.750, 0.088, 0.425]])
...
def acd_model(input_od, lambda_p=0.002, lambda_b=10, lambda_e=1, eta=0.6, gamma=0.5):
    ...
    w = [
        tf.Variable(1.0, dtype="float32"),
        tf.Variable(1.0, dtype="float32"),
        tf.constant(1.0),
    ]

    sca_mat = tf.stack(
        (tf.cos(alpha) * tf.sin(beta), tf.cos(alpha) * tf.cos(beta), tf.sin(alpha)),
        axis=1,
    )
    tf.compat.v1.disable_eager_execution()
    cd_mat = tf.compat.v1.matrix_inverse(sca_mat)
