...
class BaseGraphBuilder(PipelineStep):
    ...
    def _get_node_centroids(self, instance_map: np.ndarray) -> np.ndarray:
    ...
    def _set_node_centroids(self, centroids: np.ndarray, graph: dgl.DGLGraph) -> None:
    ...
    def _set_node_features(
        self, features: torch.Tensor, image_size: Tuple[int, int], graph: dgl.DGLGraph
    ) -> None:
    ...
    @abstractmethod
    def _build_topology(
        self, instance_map: np.ndarray, centroids: np.ndarray, graph: dgl.DGLGraph
    ) -> None:
...
class RAGGraphBuilder(BaseGraphBuilder):
    ...
    def __init__(
        self, device: torch.device, kernel_size: int = 3, hops: int = 1, **kwargs
    ) -> None:
    ...
    def _build_topology(
        ...
        regions = regionprops(instance_map)
        instance_map = torch.tensor(instance_map, device=self.device)
        # kernel = np.ones((self.kernel_size, self.kernel_size), np.uint8)
        adjacency = torch.zeros([len(regions), len(regions)], device=self.device)
        for instance_id in np.arange(1, len(regions) + 1):
            mask = (instance_map == instance_id).to(torch.float32).unsqueeze(0)
            dilation = self.max_pool(mask)
            boundary = dilation - mask
            idx = torch.unique(instance_map[boundary.squeeze().bool()])
            instance_id -= 1  # because instance_map id starts from 1
            idx -= 1  # because instance_map id starts from 1
            adjacency[instance_id, idx.cpu().numpy()] = 1
            adjacency[idx.cpu().numpy(), instance_id] = 1

        edge_list = torch.nonzero(adjacency, as_tuple=True)
        graph.add_edges(edge_list[0].cpu(), edge_list[1].cpu())
